...
static const struct snd_kcontrol_new oplus_pa_manager_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Amp_Force_Mute_Switch", (oplus_amp_control_enum)[5], ext_amp_force_mute_get, ext_amp_force_mute_set),
};

static const struct snd_kcontrol_new oplus_spkr_switch[] = {
	SOC_DAPM_SINGLE("Switch", SND_SOC_NOPM, 0, 1, 0)
};

static const struct snd_soc_dapm_widget oplus_analog_pa_manager_dapm_widgets[] = {
	SND_SOC_DAPM_MIXER("Speaker_Amp", SND_SOC_NOPM, 0, 0,
			   oplus_spkr_switch, ARRAY_SIZE(oplus_spkr_switch)),
	SND_SOC_DAPM_SPK("OPLUS_SPKR_DRV", oplus_spkr_pa_event),

	SND_SOC_DAPM_OUTPUT("OPLUS_SPKR_OUT"),
};

static const struct snd_soc_dapm_route oplus_analog_pa_manager_dapm_map[] = {
	{"Speaker_Amp", "Switch", "BE_OUT"},
	{"OPLUS_SPKR_DRV", NULL, "Speaker_Amp"},
	{"OPLUS_SPKR_OUT", NULL, "OPLUS_SPKR_DRV"},
};
...
int speaker_l_mfr_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_warning("%s, %d, ucontrol->value.integer.value[0] = %d\n", __func__, __LINE__, ucontrol->value.integer.value[0]);
